...
- **Docker Compose** 是一个用于**定义和运行多容器 Docker 应用程序**的工具。它允许你使用一个 YAML 文件来配置应用所需的所有服务（容器）、网络和数据卷。然后，只需一个简单的命令，就可以根据这个配置文件创建并启动所有服务
...
## `docker-compose.yml`常用属性

- `docker-compose.yml` 文件是 Docker Compose 的核心，它以声明式的方式定义了应用的所有组件。理解其中的常用属性是高效使用 Docker 进行应用编排的关键

### 1. 顶级键

- 这些是 `docker-compose.yml` 文件最外层的配置项

| 属性       | 作用                                                         | 示例             |
| ---------- | ------------------------------------------------------------ | ---------------- |
| `version`  | (已废弃) 曾经用于指定 Compose 文件的版本。在现代 Docker Compose 中，此字段是可选的且不再起作用 | `version: '3.8'` |
| `services` | **[核心]** 定义应用的所有服务（容器）。这是文件最重要的部分  | `services:`      |
| `networks` | 定义供服务连接的自定义网络，可以控制网络类型、驱动和选项     | `networks:`      |
| `volumes`  | 定义供服务使用的命名数据卷，方便管理和重用                   | `volumes:`       |
| `configs`  | 定义应用的配置信息，可以挂载到服务中                         | `configs:`       |
| `secrets`  | 定义敏感数据（如密码、API密钥），以更安全的方式挂载到服务中  | `secrets:`       |
...
| 属性             | 作用                                                         | 示例                                                         |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `image`          | 指定服务使用的 Docker 镜像<br />Docker 会先在本地查找，如果找不到则会从 Docker Hub 拉取 | `image: node:18-alpine`                                      |
| `build`          | 指定用于构建镜像的 `Dockerfile`<br />如果同时指定了 `image` 和 `build`，构建出的镜像会使用 `image` 指定的名称和标签 | `build: ./backend` 或 `build: { context: ./path, dockerfile: Dockerfile-dev }` |
| `container_name` | 指定容器的自定义名称<br />如果不指定，Docker Compose 会生成一个默认名称 | `container_name: my-web-server`                              |
...
| 属性       | 作用                                                         | 示例                     |
| ---------- | ------------------------------------------------------------ | ------------------------ |
| `ports`    | 将宿主机的端口映射到容器的端口。格式为 `"HOST:CONTAINER"`    | `ports: - "8080:80"`     |
| `expose`   | 仅在容器网络内部暴露端口，供其他服务访问，而**不映射到宿主机** | `expose: - "3000"`       |
| `networks` | 将服务连接到一个或多个网络                                   | `networks: - my-network` |
...
| 属性      | 作用                               | 示例                                                         |
| --------- | ---------------------------------- | ------------------------------------------------------------ |
| `volumes` | 挂载数据卷或绑定挂载。语法非常灵活 | `volumes:<br>- my-data:/var/lib/mysql<br>- ./src:/app<br>- /path/on/host:/path/in/container:ro` |
...
| 属性          | 作用                                                      | 示例                                                       |
| ------------- | --------------------------------------------------------- | ---------------------------------------------------------- |
| `command`     | 覆盖容器启动时默认执行的命令 (即 `Dockerfile` 中的 `CMD`) | `command: ["npm", "run", "dev"]` 或 `command: npm run dev` |
| `entrypoint`  | 覆盖容器的入口点 (即 `Dockerfile` 中的 `ENTRYPOINT`)      | `entrypoint: /docker-entrypoint.sh`                        |
| `working_dir` | 指定容器内的工作目录                                      | `working_dir: /app`                                        |
| `user`        | 指定运行容器内进程的用户名或 UID                          | `user: "node"`                                             |
...
| 属性          | 作用                                 | 示例                                                         |
| ------------- | ------------------------------------ | ------------------------------------------------------------ |
| `environment` | 设置环境变量。可以是键值对列表或字典 | `environment:<br>- DEBUG=true<br>- DB_HOST=database`         |
| `env_file`    | 从一个文件加载环境变量               | `env_file: .env.production` 或 `env_file: [".env.common", ".env.prod"]` |
...
| 属性          | 作用                                                         | 示例                                                         |
| ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `restart`     | 定义容器退出时的重启策略。常用值：`no` (默认), `always`, `on-failure` | `restart: always`                                            |
| `depends_on`  | 定义服务间的依赖关系，控制启动顺序。**注意：只保证启动顺序，不保证服务内的应用已准备就绪** | `depends_on: [db, redis]`                                    |
| `healthcheck` | 定义一个健康检查命令，Docker 会定期运行它来判断容器是否处于“健康”状态 | `healthcheck: { test: ["CMD", "curl", "-f", "http://localhost"], interval: 30s, timeout: 10s, retries: 3 }` |
...
```yaml
# ---------------------------------------------------------------------------
# Docker Compose 配置文件 - 综合示例
#
# 本文件定义了一个典型的多服务应用，包含一个 Web 应用和一个数据库。
# 每一行配置都附有详细的注释，用于解释其作用。
# ---------------------------------------------------------------------------

# [顶级键] 'services' - 定义应用的所有服务（容器）
...
- **常用选项**:
  ...
  - `-t`, `--tag <名称:标签>`: **(最常用)** 
    ...
    - 如果在使用 `docker build` 构建镜像时，不指定版本号（也就是 Tag），Docker 会自动为这个镜像打上一个默认的标签：**`:latest`**，这会导致很严重的问题：
      - 虽然 `latest` 听起来像是“最新的”，但它并**不是一个会自动更新的智能指针**。这里它是一个普通的名字，就像 `:v1`、`:prod` 一样，**`latest` 标签在这里仅仅指向你最后一个用 `latest` 标签构建的那个镜像**
      - 在这种情况下，拉取镜像的时候，如果使用`:latest`这个便签拉取，原本本意是拉取到最新的镜像，但是因为这里的原因，会导致拉取到这个标签名字为`:latest`的镜像
...
- **核心作用**
  ...
  - 创建一个自定义的 Docker 网络。这是构建多容器应用（例如 Web 应用 + 数据库）的**关键第一步**。
    ...
    虽然 Docker 默认提供了一个名为 `bridge` 的网络，但**最佳实践**是为每个应用或项目创建自己的**自定义桥接网络 (custom bridge network)**

    这样做能提供更好的网络隔离，并且最重要的是，它开启了基于容器名称的**自动服务发现**功能
