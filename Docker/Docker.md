...
- `docker build` 命令会读取 Dockerfile 中的指令，并按顺序执行。其核心机制是**分层（Layers）**
  1. **基础镜像 (Base Image):** 每个 Dockerfile 的第一条有效指令通常是 `FROM`，它指定了一个父镜像或基础镜像。构建过程从这个基础镜像开始。
  2. **创建中间层:** Dockerfile 中的每一条指令（如 `RUN`, `COPY`, `ADD` 等）都会在当前镜像层的顶部创建一个新的、可写的**临时容器**。指令在该容器内执行，执行完毕后，文件系统的变更会被提交（commit）成一个新的、只读的**镜像层（Image Layer）**。
...
- Docker 提供了三种将数据从宿主机挂载到容器内部的机制：
  ...
  - **Volumes (数据卷)**：**官方最推荐的方式**。由 Docker 进行管理，存储在宿主机文件系统的一个特定部分（通常是 `/var/lib/docker/volumes/`），与宿主机的核心功能完全隔离
