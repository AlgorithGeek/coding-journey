...
枚举是Java 5引入的一种特殊的类，用于定义一组 **类型安全、数量固定** 的常量集合
...
枚举本质上是一个继承自 `java.lang.Enum` 的特殊类。`enum` 关键字只是一个语法糖，编译器在背后做了大量工作，生成了很多代码：
...
public final class Color extends Enum<Color> {
    ...
    // 1. 实例：每一个枚举常量，都是该类的一个 public static final 实例
    public static final Color RED = new Color("RED", 0);
    public static final Color GREEN = new Color("GREEN", 1);
    public static final Color BLUE = new Color("BLUE", 2);
    
    // 2. 构造器：构造器必须是 private (或包私有)，由编译器强制执行，防止外部 new
    private Color(String name, int ordinal) {
        super(name, ordinal); 		// name 和 ordinal 传入父类 java.lang.Enum
    }

    // 3. 静态 $VALUES 数组：存储所有实例，供 values() 方法使用
    private static final Color[] $VALUES = {RED, GREEN, BLUE};
    
    // 4. 静态 values() 方法：由编译器自动插入，返回所有实例的数组
    public static Color[] values() {
        return $VALUES.clone(); 		// clone() 是为了防止外部修改数组内容
        								// clone() : 创建并返回一个新的数组实例，这个新数组包含了原始数组中所有元素的“副本”
    }
    
    // 5. 静态 valueOf(String) 方法：由编译器自动插入，用于按名称查找实例
...
- [ ] 接口
  ...
  >1. **`Comparable` 接口**
  >   - `java.lang.Enum` 类自动实现了 `Comparable` 接口。
  >   - 比较的依据是 `ordinal()`，即常量的**声明顺序**。(`RED.compareTo(GREEN)` < 0)。
  >2. **`Serializable` 接口**
  >   - `java.lang.Enum` 类自动实现了 `Serializable` 接口。
  >   - JVM会特殊处理枚举的序列化和反序列化，确保反序列化后得到的实例与JVM中的**原始实例是同一个** (即 `==` 成立)，防止了通过序列化破坏单例。
...
1. 枚举常量列表（`MERCURY(...)`）**必须 ** 定义在文件的最顶部，在任何字段或方法之前
2. 构造器 **必须** 是 `private` 或 包私有(默认) 的。这保证了外部无法 `new` 出新的实例
3. 强烈推荐将所有字段声明为 `final`，以保证枚举实例的不可变性
...
### 4.2 枚举中的抽象方法
 ...
 `enum` 最强大的用法之一，它允许**每个枚举常量拥有自己独特的行为实现**
